...
def se_block_2(inputs, reduction=16):
    ...
    se = keras.layers.Dense(filters // reduction, activation=None, use_bias=False)(se)
    se=keras.layers.LeakyReLU()(se)
...
def bottleneck(inputs, filters, kernel, t, s, r=False, se=False):
    channel_axis = -1 #1 if K.image_data_format() == "channels_first" else -1
    tchannel = K.int_shape(inputs)[channel_axis] * t
    Z1 = conv_block(inputs, tchannel, 1, 1, "same")
    if s==2:
        Z1=keras.layers.MaxPooling2D(pool_size=(2,2))(Z1)
    else:
...
def UMFacenet(input_shape=(256, 256, 3), dropout=0.5, filters=[64, 128, 256, 512], use_se=True):
    ...
    up3 = keras.layers.Conv2DTranspose(filters[2], (3, 3), strides=(2, 2), padding="same", name='up3_trans2d')(M3)
    ...
    #d2 = keras.layers.Conv2DTranspose(filters[1], (3, 3), strides=(2, 2), padding="same", name='d2_trans2d_1')(up2d)
    ...
    #d3 = keras.layers.Conv2DTranspose(filters[2], (3, 3), strides=(4, 4), padding="same", name='d3_trans2d_1')(up3d)
    d3 = keras.layers.UpSampling2D(size=(8, 8))(up3d)
    #d3 = keras.layers.Conv2DTranspose(filters[2], (3, 3), strides=(2, 2), padding="same", name='d3_trans2d_2')(d3)
    ...
    #d4 = keras.layers.Conv2DTranspose(filters[0], (3, 3), strides=(2, 2), padding="same")(M0)
    d4=keras.layers.UpSampling2D(size=(2,2))(M0)
    ...
    d5 = keras.layers.UpSampling2D(size=(2, 2))(M)
...
if __name__ == '__main__':
    '''
   Total params: 1,615,048
   Trainable params: 1,592,392
   Non-trainable params: 22,656
   -SE

   Total params: 1,571,140
   Trainable params: 1,548,484
   Non-trainable params: 22,656

    '''
    # model=UMFacenet(input_shape=(256,256,3))
    model = UMFacenet(input_shape=(256, 256, 3), filters=[32, 64, 128, 256], use_se=True)
    model.summary()
    keras.utils.plot_model(model, 'UMFacenetS0-SE.png', show_shapes=True)